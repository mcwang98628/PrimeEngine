...
int ClientGame::runGameFrame()
{
	...
	// create PRE_PHYSICS_UPDATE event
	{
        Handle hpupdateEvent("EVENT", sizeof(Event_PRE_PHYSICS_UPDATE));
        Event_PRE_PHYSICS_UPDATE *pUpdateEvent = new(hpupdateEvent) Event_PRE_PHYSICS_UPDATE ;
        
        pUpdateEvent->m_frameTime = m_frameTime;
        
        //Push PHYSICS_UPDATE event onto general queue
        Events::EventQueueManager::Instance()->add(hpupdateEvent, Events::QT_GENERAL);
    }
	// create POST_PHYSICS_UPDATE event
	{
    	Handle hpupdateEvent("EVENT", sizeof(Event_POST_PHYSICS_UPDATE));
    	Event_POST_PHYSICS_UPDATE *pUpdateEvent = new(hpupdateEvent) Event_POST_PHYSICS_UPDATE ;
    ...
    while (!gqh.getObject<Events::EventQueue>()->empty())
    {
		...
		else if (Event_PRE_PHYSICS_UPDATE::GetClassId() == pGeneralEvt->getClassId())
        {
            m_pContext->getPhysicsManager()->handleEvent(pGeneralEvt);

			// physics update method
        }
		else if (Event_POST_PHYSICS_UPDATE::GetClassId() == pGeneralEvt->getClassId())
		{
			m_pContext->getPhysicsManager()->handleEvent(pGeneralEvt);
