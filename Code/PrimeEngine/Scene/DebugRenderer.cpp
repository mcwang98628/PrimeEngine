...
void DebugRenderer::createBoundingBox(Vector3 minPoint, Vector3 maxPoint, Matrix4x4 rootMatrix) {
	const int boxVertices = 8;

	Vector3 pt0 = Vector3(minPoint.getX(), minPoint.getY(), minPoint.getZ());
	Vector3 pt1 = Vector3(maxPoint.getX(), minPoint.getY(), minPoint.getZ());
	Vector3 pt2 = Vector3(minPoint.getX(), minPoint.getY(), maxPoint.getZ());
	Vector3 pt3 = Vector3(maxPoint.getX(), minPoint.getY(), maxPoint.getZ());

	Vector3 pt4 = Vector3(minPoint.getX(), maxPoint.getY(), minPoint.getZ());
	Vector3 pt5 = Vector3(maxPoint.getX(), maxPoint.getY(), minPoint.getZ());
	Vector3 pt6 = Vector3(minPoint.getX(), maxPoint.getY(), maxPoint.getZ());
	Vector3 pt7 = Vector3(maxPoint.getX(), maxPoint.getY(), maxPoint.getZ());
	...
	for (int i = 0; i < 8; i++)
	{
		...
		pointMatrix[i] = rootMatrix * pointMatrix[i];
	...
	Vector3 linepts[] = { 
		...
		pts[0], color, pts[2], color, 
		pts[0], color, pts[4], color, 
		pts[1], color, pts[3], color, 
		pts[2], color, pts[3], color,
		pts[7], color, pts[6], color,
		pts[7], color, pts[5], color,
		pts[7], color, pts[3], color,
		pts[6], color, pts[4], color,
		pts[4], color, pts[5], color,
		pts[2], color, pts[6], color,
		pts[1], color, pts[5], color,
	};

	DebugRenderer::Instance()->createLineMesh(false, Matrix4x4(),  &linepts[0].m_x, 24, 0);// send event while the array is on the stack
}
