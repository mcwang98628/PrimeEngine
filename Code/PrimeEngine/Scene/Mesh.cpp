...
void Mesh::loadFromMeshCPU_needsRC(MeshCPU &mcpu, int &threadOwnershipMask)
{
	...
	*/
	...
	PositionBufferCPU *pPos = m_hPositionBufferCPU.getObject<PositionBufferCPU>();
	...
	for (int i = 0; i < pPos->m_values.m_size / 3; ++i)
	{
		float x = pPos->m_values[i * 3];
		float y = pPos->m_values[i * 3 + 1];
		float z = pPos->m_values[i * 3 + 2];
		if (x < minX)
		{
			minX = x;
		}
