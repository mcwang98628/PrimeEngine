...
void SceneNode::setNextTarget(Vector3 m_target)
{
	Handle pPhysicsHandle = getFirstComponentHandle<PhysicsComponent>();
	if (!pPhysicsHandle.isValid())
	{
		Handle sceneNodeHandle = getFirstComponentHandle<SceneNode>();
		SceneNode* pSN = sceneNodeHandle.getObject<SceneNode>();
		
		pPhysicsHandle = pSN->getFirstComponentHandle<PhysicsComponent>();
	}
	PhysicsComponent* pComp = pPhysicsHandle.getObject<PhysicsComponent>();
	pComp->m_target = m_target;
}
