...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
    ...
    if (true && pMeshCaller->m_performBoundingVolumeCulling)
    {
        ...
        for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
        {
			...
			Handle hPSN = pInst->getFirstParentByType<SceneNode>();
			...
			if (!hPSN.isValid())
			{
				...
				{
					hPSN = pParentSkelInstance->getFirstParentByType<SceneNode>();
				}
			}
			SceneNode *pSN = hPSN.getObject<SceneNode>();
			CameraSceneNode *cameraSceneNode = CameraManager::Instance()->getActiveCamera()->getCamSceneNode();

			if (pSN)
			{
				Mesh::BoundingBox bBox = pMeshCaller->m_aabb;

				Matrix4x4 baseMatrix = pSN->m_base;

				float minX = bBox.min_X;
				float minY = bBox.min_Y;
				float minZ = bBox.min_Z;
				float maxX = bBox.max_X;
				float maxY = bBox.max_Y;
				float maxZ = bBox.max_Z;

				
				Vector3 pt0 = Vector3(minX, minY, minZ);
				Vector3 pt1 = Vector3(maxX, minY, minZ);
				Vector3 pt2 = Vector3(minX, minY, maxZ);
				Vector3 pt3 = Vector3(maxX, minY, maxZ);

				Vector3 pt4 = Vector3(minX, maxY, minZ);
				Vector3 pt5 = Vector3(maxX, maxY, minZ);
				Vector3 pt6 = Vector3(minX, maxY, maxZ);
				Vector3 pt7 = Vector3(maxX, maxY, maxZ);

				Vector3 pts[] = {pt0, pt1, pt2, pt3, pt4, pt5, pt6, pt7};
				Matrix4x4 pointMatrix[8];
				for (int i = 0; i < 8; i++)
				{
					pointMatrix[i].loadIdentity();
					pointMatrix[i].setPos(pts[i]);
					pointMatrix[i] = pSN->m_worldTransform * pointMatrix[i];
					pts[i] = pointMatrix[i].getPos();
				}

				bool isOneBoxPointInFrustum = true;
				for (int i = 0; i < 8; i++)
				{
					if (cameraSceneNode->m_frustum.isInFrustum(pts[i])) {
						isOneBoxPointInFrustum = true;
					}
				}

				// if (1)
				// {
				// 	Vector3 nearColor(0.f, 0.f, 1.f);
				// 	Vector3 farColor(0.f, 1.f, 1.f);
				// 	Vector3 color(1.f, 0.f, 0.f);

				// 	Vector3 linepts[] = { 
				// 		cameraSceneNode->m_frustum.nearRightBot, nearColor, cameraSceneNode->m_frustum.nearRightTop, nearColor,
				// 		cameraSceneNode->m_frustum.nearRightTop, nearColor, cameraSceneNode->m_frustum.nearLeftTop, nearColor,
				// 		cameraSceneNode->m_frustum.nearLeftBot, nearColor, cameraSceneNode->m_frustum.nearRightBot, nearColor, 
				// 		cameraSceneNode->m_frustum.nearLeftBot, nearColor, cameraSceneNode->m_frustum.nearLeftTop, nearColor, 
				// 		cameraSceneNode->m_frustum.nearLeftBot, nearColor, cameraSceneNode->m_frustum.nearRightTop, nearColor,
				// 		cameraSceneNode->m_frustum.nearRightBot, nearColor, cameraSceneNode->m_frustum.nearLeftTop, nearColor,
						// cameraSceneNode->m_frustum.nearLeftBot, color, cameraSceneNode->m_frustum.farLeftBot, color, 
						// cameraSceneNode->m_frustum.farRightTop, farColor, cameraSceneNode->m_frustum.farRightBot, farColor, 
						// cameraSceneNode->m_frustum.farRightTop, farColor, cameraSceneNode->m_frustum.farLeftBot, farColor,
						// cameraSceneNode->m_frustum.farLeftTop, farColor, cameraSceneNode->m_frustum.farRightBot, farColor,
						// cameraSceneNode->m_frustum.farRightTop, color, cameraSceneNode->m_frustum.nearRightTop, color,
						// cameraSceneNode->m_frustum.farRightTop, farColor, cameraSceneNode->m_frustum.farLeftTop, farColor,
						// cameraSceneNode->m_frustum.farLeftTop, farColor, cameraSceneNode->m_frustum.farLeftBot, farColor,
						// cameraSceneNode->m_frustum.farLeftTop, color, cameraSceneNode->m_frustum.nearLeftTop, color,
						// cameraSceneNode->m_frustum.farLeftBot, farColor, cameraSceneNode->m_frustum.farRightBot, farColor,
						// cameraSceneNode->m_frustum.farRightBot, color, cameraSceneNode->m_frustum.nearRightBot, color,
						
					// };
					// DebugRenderer::Instance()->createLineMesh(false, Matrix4x4(),  &linepts[0].m_x, 12, 0);
				// }

				// ++pMeshCaller->m_numVisibleInstances;
				// pInst->m_culledOut = false;
				if (!isOneBoxPointInFrustum)
				{
					pInst->m_culledOut = !isOneBoxPointInFrustum;
				}
				else 
				{
					++pMeshCaller->m_numVisibleInstances;
				...
				if (isOneBoxPointInFrustum) {
					// DebugRenderer::Instance()->createBoundingBox(Vector3(minX, minY, minZ), Vector3(maxX, maxY, maxZ), pSN->m_worldTransform);
				}
			}
        }
    }
