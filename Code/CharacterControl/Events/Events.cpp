...
int Event_CreateSoldierNPC::l_Construct(lua_State* luaVM)
{
	...
	{
		// have enemy name
		enemyName = lua_tostring(luaVM, -numArgs--);
		StringOps::writeToString(enemyName, pEvt->m_enemy, 32);
	}
	else
		// ignore
		numArgs--;
...
// PE_IMPLEMENT_CLASS1(Event_CreateEnemyNPC, PE::Events::Event_CREATE_MESH);
// int Event_CreateEnemyNPC::l_Construct(lua_State* luaVM)
// {
// 	PE::Handle h("EVENT", sizeof(Event_CreateEnemyNPC));
...
// 	// this function should only be called frm game thread, so we can use game thread thread owenrship mask
// 	Event_CreateEnemyNPC *pEvt = new(h) Event_CreateEnemyNPC(pContext->m_gameThreadThreadOwnershipMask);
