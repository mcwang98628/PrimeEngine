...
namespace CharacterControl {
	...
	namespace Events {
		...
		PE_IMPLEMENT_CLASS1(SoldierNPCMovementSM_Event_ENEMY_DETECTED, Event);

		SoldierNPCMovementSM_Event_ENEMY_DETECTED::SoldierNPCMovementSM_Event_ENEMY_DETECTED(SceneNode* targetNode /* = enemyObject */)
			: m_enemyNode(targetNode)
		{}
	}
	...
	namespace Components {
		...
		void SoldierNPCMovementSM::do_SoldierNPCMovementSM_Event_MOVE_TO(PE::Events::Event* pEvt)
		{
			...
			OutputDebugStringA("PE: PROGRESS: SoldierNPCMovementSM::do_SoldierNPCMovementSM_Event_MOVE_TO() Received: running: ");
		...
		void SoldierNPCMovementSM::do_SoldierNPCMovementSM_Event_ENEMY_DETECTED(PE::Events::Event* pEvt)
		{
			SoldierNPCMovementSM_Event_ENEMY_DETECTED* pRealEvt = (SoldierNPCMovementSM_Event_ENEMY_DETECTED*)(pEvt);
			
			m_enemyNode = pRealEvt->m_enemyNode;

			Events::SoldierNPCAnimSM_Event_STAND_AND_SHOOT Evt;
			...
			if (m_state == WALKING_TO_TARGET || m_state == RUNNING_TO_TARGET)
			{
				...
				if (pSN)
				{
					...
					if (dsqr > 0.01f)
					{
						...
						float speed = (m_state == WALKING_TO_TARGET) ? 1.4f : 3.f;
						...
						Vector3 curPos = pSN->m_base.getPos();
						Vector3 nextPos = curPos + dir * dist;
						pSN->setNextTarget(nextPos);
