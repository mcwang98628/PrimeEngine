...
// void ClientGameObjectManagerAddon::do_CreateEnemyNPC(PE::Events::Event *pEvt)
// {
// 	assert(pEvt->isInstanceOf<Event_CreateEnemyNPC>());

// 	Event_CreateEnemyNPC *pTrueEvent = (Event_CreateEnemyNPC*)(pEvt);

// 	createEnemyNPC(pTrueEvent);
// }

// void ClientGameObjectManagerAddon::createEnemyNPC(Vector3 pos, int &threadOwnershipMask)
// {
// 	Event_CreateEnemyNPC evt(threadOwnershipMask);
// 	evt.m_pos = pos;
// 	evt.m_u = Vector3(1.0f, 0, 0);
// 	evt.m_v = Vector3(0, 1.0f, 0);
// 	evt.m_n = Vector3(0, 0, 1.0f);
...
// void ClientGameObjectManagerAddon::createEnemyNPC(Event_CreateEnemyNPC *pTrueEvent)
// {
// 	PEINFO("CharacterControl: GameObjectManagerAddon: Creating CreateSoldierNPC\n");

// 	PE::Handle hEnemyNPC("EnemyNPC", sizeof(EnemyNPC));
// 	SoldierNPC *pEnemyNPC = new(hEnemyNPC) EnemyNPC(*m_pContext, m_arena, hEnemyNPC, pTrueEvent);
// 	pEnemyNPC->addDefaultComponents();

// 	// add the soldier as component to the ObjecManagerComponentAddon
// 	// all objects of this demo live in the ObjecManagerComponentAddon
// 	addComponent(hEnemyNPC);
// }
...
SoldierNPC *ClientGameObjectManagerAddon::getEnemy(const char *name)
{
	PE::Handle *pHC = m_components.getFirstPtr();
	...
	for (PrimitiveTypes::UInt32 i = 0; i < m_components.m_size; i++, pHC++) // fast array traversal (increasing ptr)
	{
		...
		{
			SoldierNPC *pS = (SoldierNPC *)(pC);
			if (strcmp(pS->m_npcType, "Enemy") == 0) {
				return pS;
			}
		}
	}
