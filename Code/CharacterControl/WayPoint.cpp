...
int Event_CREATE_WAYPOINT::l_Construct(lua_State* luaVM)
{
	...
	const char* nextWayPointName1 = (const char*)lua_tostring(luaVM, -numArgs--);
	const char* nextWayPointName2 = lua_tostring(luaVM, -numArgs--);

	float positionFactor = 1.0f / 100.0f;
	Vector3 pos, next, u, v, n;
	...
	StringOps::writeToString(nextWayPointName1, pEvt->m_nextChoice1, 32);
	StringOps::writeToString(nextWayPointName2, pEvt->m_nextChoice2, 32);
...
: Component(context, arena, hMyself)
{
	...
	StringOps::writeToString(pEvt->m_nextChoice1, m_nextChoice1, 32);
	StringOps::writeToString(pEvt->m_nextChoice2, m_nextChoice2, 32);
