...
struct ClientGameObjectManagerAddon : public GameObjectManagerAddon
{
	...
	// PE_DECLARE_IMPLEMENT_EVENT_HANDLER_WRAPPER(do_CreateEnemyNPC);
	// virtual void do_CreateEnemyNPC(PE::Events::Event *pEvt);
	...
	// void createEnemyNPC(Vector3 pos, int &threadOwnershipMask);
	// void createEnemyNPC(Events::Event_CreateEnemyNPC *pTrueEvent);
	...
	SoldierNPC *getEnemy(const char *name);
};
